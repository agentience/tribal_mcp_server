...
from mcp_server_tribal.services.storage_interface import StorageInterface
...
class MockStorage(StorageInterface):
    ...
    async def update_error(
        self, error_id: uuid.UUID, error: ErrorRecord
...
def test_search_by_exact_match(client, sample_error_records):
    ...
    response = client.get("/api/v1/errors/", params={"error_type": "ImportError"})
    assert response.status_code == 200
    ...
    # Search by language
    response = client.get("/api/v1/errors/", params={"language": "python"})
    assert response.status_code == 200
    results = response.json()
    assert len(results) > 0
    ...
    # Search by framework
    response = client.get("/api/v1/errors/", params={"framework": "fastapi"})
    assert response.status_code == 200
    results = response.json()
    assert len(results) > 0
...
def test_search_by_similar_message(client, sample_error_records):
    ...
    response = client.get(
        ...
        params={"query": "cannot import FastAPI module", "max_results": 2},
    )
    assert response.status_code == 200
    results = response.json()
    assert len(results) > 0
    ...
    assert any(
        ...
        and "fastapi" in r["context"]["error_message"].lower()
        for r in results
    )
...
def test_real_world_workflow(client, sample_error_records):
    ...
    response = client.get(
        "/api/v1/errors/similar/", params={"query": query, "max_results": 3}
    )
    assert response.status_code == 200
    results = response.json()
    assert len(results) > 0
    ...
    for result in results:
        if (
            ...
            and "pandas" in result["context"].get("framework", "").lower()
            and "astype" in result["solution"].get("code_fix", "").lower()
        ):
