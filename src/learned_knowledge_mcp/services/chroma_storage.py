...
class ChromaStorage(StorageInterface):
    ...
    async def get_error(self, error_id: UUID) -> Optional[ErrorRecord]:
        ...
        try:
            result = self.collection.get(ids=[str(error_id)], include=["documents", "metadatas"])
    ...
    async def update_error(self, error_id: UUID, error: ErrorRecord) -> Optional[ErrorRecord]:
